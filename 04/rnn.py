...
class TinyLSTM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, states: torch.Tensor | None = None):
        ...
        device = x.device
        if states is None:
            h_states = torch.zeros(self.num_layers, batch_size, self.hidden_size)
            c_states = torch.zeros(self.num_layers, batch_size, self.hidden_size)
        else:
        ...
        for t in range(seq_len):
            ...
            for layer_idx in range(self.num_layers):
                ...
                i_t = torch.sigmoid(layer_input @ layer['W_xi'] + h_prev @ layer['W_hi'] + layer['b_i']) # pyright: ignore
                f_t = torch.sigmoid(layer_input @ layer['W_xf'] + h_prev @ layer['W_hf'] + layer['b_f']) # pyright: ignore
                o_t = torch.sigmoid(layer_input @ layer['W_xo'] + h_prev @ layer['W_ho'] + layer['b_o']) # pyright: ignore
                g_t = torch.tanh(layer_input @ layer['W_xc'] + h_prev @ layer['W_hc'] + layer['b_c']) # pyright: ignore
        ...
        outputs = torch.stack(outputs, dim=1)  # (batch_size, seq_len, output_size)
        h_seq_stacked = torch.stack(h_seq, dim=1)  # (num_layers, batch_size, seq_len, hidden_size)
        c_seq_stacked = torch.stack(c_seq, dim=1)  # (num_layers, batch_size, seq_len, hidden_size)
